#!/usr/bin/env pwsh
# Safe Mass Import Replacement Tool
# Usage: .\replace-imports.ps1 -OLD "old text" -NEW "new text" -BRANCH "branch-name" -EXTS "*.js,*.ts" -DRY_RUN $true
...
try {
    ...
    # Find files
    Write-Info "Finding target files..."
    $allFiles = @()
    ...
    Write-Host "📁 Found $($allFiles.Count) files" -ForegroundColor Blue
    
    # Find matches
    Write-Info "Searching for matches..."
    $matchingFiles = @()
    ...
    if ($matchingFiles.Count -eq 0) {
        Write-Host "ℹ️ No matches found for '$OLD'" -ForegroundColor Blue
        exit 0
    }
    ...
    Write-Host "📊 Total: $totalMatches matches in $($matchingFiles.Count) files" -ForegroundColor Green
    ...
    if ($DRY_RUN) {
        ...
        # Mark remaining todos as done since we completed dry-run
        Write-Host ""
        Write-Host "▶️ Next steps:" -ForegroundColor Green
        Write-Host "  .\replace-imports.ps1 -OLD '$OLD' -NEW '$NEW' -BRANCH '$BRANCH' -EXTS '$EXTS' -DRY_RUN `\$false" -ForegroundColor Cyan
        exit 0
    }
    ...
    Write-Info "Managing branch: $BRANCH"
    $branchExists = $null
    try {
    ...
    if ($branchExists) {
        Write-Host "🌿 Switching to existing branch: $BRANCH" -ForegroundColor Blue
        git checkout $BRANCH
    } else {
    ...
    if ($changedFiles) {
        ...
        Write-Host "ℹ️ No git changes detected" -ForegroundColor Blue
        exit 0
    }
    ...
    if (Test-Path "package.json") {
        ...
        if ($pkg.scripts.test -and $pkg.scripts.test -ne 'echo "No tests specified" && exit 0') {
    ...
    if (Test-Path "gs.ps1") {
        Write-Info "Using git session helper..."
    ...
    $commitMsg = "fix(imports): mass update '$OLD' → '$NEW'"
    ...
    if ($staged) {
        ...
        # Checkpoint with gs
        if (Test-Path "gs.ps1") {
            .\gs.ps1 checkpoint "Mass update imports: $OLD → $NEW"
        }
        ...
        Write-Host "  git revert $commitHash  # (if already pushed)" -ForegroundColor Gray
        Write-Host ""
        Write-Host "🔁 REVERSE REPLACEMENT:" -ForegroundColor Red
        Write-Host '  .\replace-imports.ps1 -OLD "$NEW" -NEW "$OLD" -BRANCH "revert/$BRANCH" -DRY_RUN $false' -ForegroundColor Gray
        
    } else {
        Write-Host "ℹ️ Nothing to commit" -ForegroundColor Blue
    }
