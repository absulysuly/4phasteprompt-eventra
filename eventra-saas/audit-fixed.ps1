...
.EXAMPLE
    .\audit-fixed.ps1 -DryRun
    Performs a dry run audit without making any changes.

.EXAMPLE
    .\audit-fixed.ps1 -FoundationName "milestone-v2" -BackupMethods @("branch", "tag")
...
function Invoke-Action {
    ...
    if ($DryRun) {
        ...
        try {
            ...
            Write-ErrorMsg "Failed to execute: $Description - $($_.Exception.Message)"
            return $false
        }
    }
}
...
function Search-TodoPatterns {
    ...
    try {
        ...
        if ($gitGrepOutput -and $LASTEXITCODE -eq 0) {
            ...
            $maxDisplay = [Math]::Min(19, $script:TodoMatches.Count - 1)
            for ($i = 0; $i -le $maxDisplay; $i++) {
                Write-Host "  $($script:TodoMatches[$i])" -ForegroundColor Gray
            }
...
function New-BaselineReport {
    ...
    $reportLines = @(
        ...
        "**Current branch:** $script:CurrentBranch",
        "**Last commit:** $lastCommit",
        "**Has uncommitted changes:** $hasChanges",
        "",
        "## Tests",
        "- **Test runner present:** $testRunner",
        ...
        "",
        "## Planned Snapshot(s)",
        "- **Methods:** $($BackupMethods -join ', ')",
        "- **Foundation name:** $FoundationName$dryRunNote",
        "",
        ...
        "git branch -D $FoundationName",
        "",
        "# Delete remote foundation branch (if pushed)",
        "git push $PushRemote --delete $FoundationName",
        "",
        "# Delete foundation tag (if created)",
        "git tag -d $FoundationName",
        "git push $PushRemote --delete $FoundationName",
        "``````",
        "",
        ...
        "``````bash",
        "# From branch",
        "git checkout $FoundationName",
        "",
        "# From tag",
        "git checkout tags/$FoundationName-$script:Timestamp -b restore-$FoundationName",
        "",
        "# From bundle", 
        "git clone --branch $FoundationName foundation-$FoundationName-$script:Timestamp.bundle restore-dir",
        "",
        "# From archive",
        "# Extract foundation-$FoundationName-$script:Timestamp.zip to desired location",
        "``````",
        "",
        "---",
        "*Report generated by audit-fixed.ps1 on $(Get-Date)*"
    )
...
try {
    ...
    if (-not (Test-GitRepository)) {
        Write-ErrorMsg "Not in a git repository. cd to repo root and re-run."
        exit 2
    }
    ...
    Write-ErrorMsg "Fatal error during audit: $($_.Exception.Message)"
