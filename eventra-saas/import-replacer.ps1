# Mass Import Replacement Tool for PowerShell
# Safe one-shot workflow to mass-update import strings
...
try {
    ...
    if ($DRY_RUN) {
        ...
        Write-Host ""
        Write-Host "▶️ Next command:" -ForegroundColor Green
        $nextCmd = ".\import-replacer.ps1 -OLD '$OLD' -NEW '$NEW' -BRANCH '$BRANCH' -EXTS '$EXTS' -DRY_RUN `$false"
    ...
    if ($staged) {
        ...
        # Checkpoint with gs
        if (Test-Path "gs.ps1") {
            $gsMsg = "Mass update imports: $OLD → $NEW"
        ...
        Write-Host ""
        Write-Host "🔁 To reverse the replacement:" -ForegroundColor Red
        $reverseCmd = ".\import-replacer.ps1 -OLD '$NEW' -NEW '$OLD' -BRANCH 'revert/$BRANCH' -DRY_RUN `$false"
        Write-Host "  $reverseCmd" -ForegroundColor Gray
