...
if [ "${DB_METHOD:-}" = "dump" ] && [ -z "${DATABASE_URL:-}" ]; then
  echo "ERROR: DATABASE_URL must be set for dump restore method."
  exit 2
fi
if [ "${DB_METHOD:-}" = "rds" ]; then
  if [ -z "${RDS_SNAPSHOT_ID:-}" ] || [ -z "${RDS_RESTORE_INSTANCE_ID:-}" ] || [ -z "${AWS_REGION:-}" ]; then
    echo "ERROR: For rds method set --aws-snapshot-id, --rds-instance-id and --aws-region."
    exit 2
  fi
fi
if [ -z "${DEPLOY_COMMIT:-}" ] && [ -z "${PREVIOUS_COMMIT:-}" ]; then
  echo "ERROR: Provide --deploy-commit or --previous-commit to revert or force-push."
  exit 2
fi
if [ -n "${DEPLOY_COMMIT:-}" ] && [ -n "${PREVIOUS_COMMIT:-}" ]; then
  echo "ERROR: Use either --deploy-commit (revert) or --previous-commit (force), not both."
  exit 2
fi

echo "=== ROLLBACK-CI SCRIPT ==="
echo "Mode: ${DRY_RUN:+DRY-RUN }${DRY_RUN:-}${FORCE:+ FORCED }"
...
if [ "$DB_METHOD" = "dump" ]; then
  ...
  if [ "$DRY_RUN" = false ]; then
    ...
    echo "Update DATABASE_URL to use $NEW_ENDPOINT before redeploying."
  fi
fi
